...
public class CustomRepositoryImpl implements CustomRepository {
    ...
    public PostResponseDTO getPost(Long postKey, Long userKey) {
        ...
        Tuple tupleFollowComment = query.select(user.userNick, comment.commContent)
                .from(follow)
                .join(comment).on(comment.userKey.eq(follow.toUserKey))
                .join(user).on(user.userKey.eq(comment.userKey))
                .where(follow.fromUserKey.eq(userKey)
        ...
        String followLikeNick = query.select(user.userNick)
                .from(follow)
                .join(postLike).on(postLike.userKey.eq(follow.toUserKey))
                .join(user).on(user.userKey.eq(postLike.userKey))
                .where(follow.fromUserKey.eq(userKey)
        ...
        PostResponseDTO postResponseDTO = query
                .select(Projections.bean(PostResponseDTO.class, post.postKey, user.userKey,
                        user.userNick, post.postContent, post.postImage, post.postDate, post.postUpdate,
                        ExpressionUtils.as(
                ...
                .from(post)
                .join(user).on(post.userKey.eq(user.userKey))
                .where(post.postKey.eq(postKey))
                .fetchOne();
        // postResponseDTO에 팔로우 사용자 닉네임 저장
        postResponseDTO.setFollowLikeNick(followLikeNick);
        postResponseDTO.setFollowCommentNick(tupleFollowComment.get(0, String.class));
        postResponseDTO.setFollowComment(tupleFollowComment.get(1, String.class));
    ...
    public List<NewsFeedDTO> getNewsFeed(Long userKey) {
        ...
        QUser user = QUser.user;
        QFollow follow = QFollow.follow;
        QUser followUser = new QUser("followUser");
        // 팔로우 사용자 목록 조회
        ...
        List<NewsFeedDTO> posts = query
                .select(Projections.bean(NewsFeedDTO.class, post.postKey, user.userKey, user.userNick, post.postContent,
                        post.postImage, post.postDate, post.postUpdate, ExpressionUtils.as(Expressions.constant(1), "newsFeedType"),
                        follow.toUserKey.as("followUserKey"), followUser.userNick.as("followUserNick"),
                        ExpressionUtils.as(
                ...
                .join(followUser).on(follow.toUserKey.eq(followUser.userKey))
                .where(post.userKey.in(followingUserIds)
                        .and(post.postDate.after(LocalDateTime.now().minusDays(7))))
                .groupBy(post.postKey, user.userKey, user.userNick, post.postContent, post.postImage, post.postDate,
                        post.postUpdate, follow.toUserKey, followUser.userNick)
