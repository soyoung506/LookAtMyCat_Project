...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            throw new BadCredentialsException("throw new not found token exception");  // 토큰을 찾지 못함 -> 로그인 안 됨
        } catch (SecurityException | MalformedJwtException e) {
            request.setAttribute("exception", JwtExceptionCode.INVALID_TOKEN.getCode());
            log.error("Invalid Token // token : {}", accessToken);
            log.error("Set Request Exception Code : {}", request.getAttribute("exception"));
            throw new BadCredentialsException("throw new invalid token exception");  // 유효하지 않는 토큰 -> 토큰이 잘못됨
        } catch (ExpiredJwtException e) {
            request.setAttribute("exception", JwtExceptionCode.EXPIRED_TOKEN.getCode());
            log.error("EXPIRED Token // token : {}", accessToken);
            log.error("Set Request Exception Code : {}", request.getAttribute("exception"));
            throw new BadCredentialsException("throw new expired token exception");  // 토큰 기한 만료 -> refresh 토큰으로 재발급 필요
        } catch (UnsupportedJwtException e) {
            request.setAttribute("exception", JwtExceptionCode.UNSUPPORTED_TOKEN.getCode());
            log.error("Unsupported Token // token : {}", accessToken);
            log.error("Set Request Exception Code : {}", request.getAttribute("exception"));
            throw new BadCredentialsException("throw new unsupported token exception");  // 지원되지 않는 토큰 -> 토큰이 잘못됨
        } catch (Exception e) {
            log.error("====================================================");
...
//                    String decodedAuthorization = URLDecoder.decode(cookie.getValue(), StandardCharsets.UTF_8); // Encode 되어 넘어간 Value 다시 Decode
//                    if (StringUtils.hasText(decodedAuthorization) && decodedAuthorization.startsWith("Bearer")) {
//                        // 헤더의 Authorization 정보에는 Bearer + access 토큰이 저장되어 있기 때문에 둘을 분리하여 access 토큰 리턴
//                        return decodedAuthorization.substring(7);
//                    }
//                }
//            }
//        }
